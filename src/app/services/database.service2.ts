...
export class DatabaseService {
  ...
  constructor(
    ...
    syncGroupedInvoices(groupedInvoice: GroupedInvoice){
      this.database.executeSql("INSERT INTO GroupedFertilizers (Year,TotalKg,TotalValue,AverageValue,FarmId) VALUES (?,?,?,?,?)",
        [
          groupedInvoice.year,
    ...
    loadFarms2() {
      ...
      return this.database.executeSql('SELECT * FROM Farms', []).then(data => {
        ...
        console.log("Select farms error");
      })
      //return farms;
    }
    ...
    loadFarms() {
      let farms: Farm[] = [];
      this.database.executeSql('SELECT * FROM Farms', []).then(data => {
